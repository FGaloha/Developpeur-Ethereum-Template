    ...
    function take() public {
        for (uint256 i; i < safes.length; ++i) {
            ...
            if (safe.owner == msg.sender && safe.amount != 0) {
                ...
                // c'est une faille de reentrancy. On devrait updater amount avant ou mettre un guard
                safe.amount = 0;
            }
        }
    }
...
// Contract to store and redeem money --> corrigé en faisant la mise a jour de amount avant l'appel et en utilisant une variable temporaire pour le call
contract StoreCorrected {
    struct Safe {
        address owner;
        uint256 amount;
    }
    ...
    function take() public {
        for (uint256 i; i < safes.length; ++i) {
            Safe storage safe = safes[i];
            uint256 myAmount = safe.amount;
            if (myAmount != 0) {
                safe.amount = 0;
                if (safe.owner == msg.sender) {
...
contract CommonCoffers {
    ...
    function deposit(address _owner) external payable {
        require(msg.value > 0);
        if (scalingFactor != 0) {
            uint256 toAdd = (scalingFactor * msg.value) /
